...
#### Jira 계정과 GitHub 계정이 다르면?

**대부분 다릅니다. 그래도 문제없고, 계정을 새로 만들거나 로그인 방식을 바꿀 필요도 없습니다.**

헷갈리는 이유는 이메일이 세 개 등장하는데 역할이 각각 다르기 때문입니다.

| 이메일 | 하는 일 | 바꿔야 하나 |
| --- | --- | --- |
| GitHub 로그인 이메일 | GitHub 계정에 접속 | ❌ 그대로 둡니다 |
| Jira 계정 이메일 | Jira가 사용자를 식별 | ❌ 그대로 둡니다 |
| **git 커밋 author 이메일** | 커밋에 기록되는 값. **Jira와 GitHub이 둘 다 이걸 보고 사람을 찾습니다** | ✅ Jira 계정 이메일로 맞춥니다 |

git은 커밋할 때 "누가 실행했는지"를 기록하지 않습니다. `user.email`에 적힌 문자열을 그대로 박을 뿐이고, 진짜 본인 것인지 검사하지도 않습니다. 그 문자열 하나를 Jira와 GitHub이 각자 열쇠로 씁니다.

- Jira: 이 이메일과 같은 Jira 계정을 찾아 마커를 실행
- GitHub: 이 이메일이 등록된 계정을 찾아 커밋을 연결

그래서 **커밋 이메일을 Jira 계정 이메일로 맞추고(②), 그 이메일을 본인 GitHub 계정에도 등록(③)** 하면 양쪽이 동시에 만족됩니다.

예시 — 로그인은 회사 이메일, Jira는 개인 이메일인 경우:

```
GitHub 로그인       hong@company.com    (그대로 로그인, 변경 없음)
Jira 계정           hong@gmail.com      (그대로, 변경 없음)
git 커밋 이메일      hong@gmail.com      ← ②에서 Jira 쪽에 맞춤

GitHub > Settings > Emails
  hong@company.com   Primary
  hong@gmail.com     ← ③에서 추가 등록 + 인증만 하면 끝
```

**하나의 GitHub 계정에 이메일을 여러 개 등록하는 건 정상 기능입니다.** 회사·개인 커밋을 한 프로필로 모으라고 만들어둔 것이라 부작용이 없습니다. Primary를 바꿀 필요도 없습니다.

#### Jira 이메일을 GitHub에 등록하고 싶지 않다면

개인 이메일을 회사 GitHub 계정에 남기고 싶지 않은 경우입니다. 이때는 둘 중 하나를 골라야 합니다.

- **Jira 계정 이메일을 바꾼다** — Jira 관리자에게 요청해 GitHub에 이미 등록된 이메일로 변경. 그러면 ③이 필요 없어집니다.
- **커밋 귀속을 포기한다** — Jira 자동화는 그대로 동작하지만, GitHub에서 커밋이 본인 계정에 연결되지 않습니다 (프로필 기여도에 안 잡힘). 기능상 문제는 없습니다.
