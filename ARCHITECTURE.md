...
단, **백엔드는 `@onnuri/shared`에서 타입만 가져온다** (`import type`). shared는 빌드 없이 TS 소스를 그대로 노출하는 패키지라(`main: src/index.ts`), 번들러(Metro)를 쓰는 모바일은 상수/유틸도 소비할 수 있지만 Node로 직접 실행되는 백엔드는 런타임 값을 import하는 순간 부팅이 깨진다 — 타입 import는 컴파일 때 지워져서 안전하다. 백엔드도 써야 하는 런타임 값이 생기면 shared에 빌드를 얹을지 결정한 뒤 진행한다 (지금은 그 필요가 DTO 검증 배열 하나뿐이라 백엔드 쪽에 두고 `satisfies`로 shared 타입과 동기화한다 — `dev-login.dto.ts` 참고).
...
* **개발용 로그인**: 소셜 SDK가 없는 웹·Expo Go에서는 실제 로그인이 불가능해 유저 기반 기능을 개발할 수 없다. 이를 위해 `POST /auth/login/dev`(이메일만으로 진짜 유저+토큰 발급)를 두고, 백엔드는 `AUTH_DEV_LOGIN=true`인 환경에서만 응답한다(아니면 404로 숨김, 운영 금지). 모바일은 로그인 화면에 "[DEV]" 역할 버튼(일반/팀장/셀장/관리자)으로 연결하되, `.env`의 `EXPO_PUBLIC_AUTH_DEV_LOGIN=true`일 때만 버튼이 보인다 (백엔드 `AUTH_DEV_LOGIN`과 짝 — 둘 다 켜야 동작).
  * 역할 확인용으로 `role`(`MEMBER`/`TEAM_LEADER`/`CELL_LEADER`/`ADMIN`)을 함께 받는다. 등급의 실체가 `isAdmin`·멤버십이므로 백엔드가 로그인하면서 관리자 플래그 또는 고정 "개발팀"/"개발셀"의 LEADER 멤버십을 멱등하게 세팅한다. 역할이 유저에 남으므로 모바일 버튼은 역할마다 고정 계정을 분리해 쓴다 (`dev@onnuri.local`, `dev-team-leader@…` 등).
