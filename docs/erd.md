...
레거시 필드는 모두 제거됨 (`User.cellName`·`teamId`·`role`은 멤버십+`isAdmin` 체계로, `password`는 소셜 로그인 전환으로). 생성·수정 시각(`createdAt`/`updatedAt`)은 다이어그램에서 대부분 생략한다 — 스키마에는 전 테이블에 있다. QR 출석·엑셀 추출은 MVP 이후 기능이다.
