...
module.exports = ({ config }) => ({
  ...
  plugins: [
    ...(config.plugins ?? []),
    // 데이터 다운로드(엑셀)의 공유 시트용 — expo install이 동적 config라 자동 추가를 못 해 수동 등록.
    "expo-sharing",
    // QR 출석 스캔용 카메라 — iOS 권한 문구는 여기서 정한다 (없으면 심사 거절 사유).
    [
      "expo-camera",
      { cameraPermission: "출석 QR을 스캔하려면 카메라 접근 권한이 필요해요." },
    ],
