...
* 굵기를 다르게 써야 하면 `shared/components/base/`의 베이스 컴포넌트 안에서, 시안 근거가 있을 때만 `font-pretendard-*`를 덧붙여 덮어쓴다. 컴포넌트가 그 결정을 감추므로 화면은 굵기를 고를 일이 없다.
* **`shared/components/base/` 밖(조합 컴포넌트·화면·기능 코드)에서는 `TEXT_STYLE`에 등록된 텍스트 스타일만 쓴다.** Tailwind 기본 `text-sm`/`text-base` 등은 클래스로 존재하지만 쓰지 않는다 — 설정이 안 막아주므로 규칙으로만 지켜진다.
* 폰트 패밀리는 `tailwind.config.js`가 통째로 교체하므로 `font-sans`/`font-serif`/`font-mono`는 클래스 자체가 없다.
* 값은 `src/shared/theme/tokens.js`의 `TEXT_STYLE`이 단일 소스. 시안에 없는 조합이 필요하면 추측하지 말고 작업자에게 요청한다.
...
* **스케일 한 칸은 4px다.** `p-4`는 16px, `w-12`는 48px로 시안 px과 그대로 맞는다. (환산 기준은 `metro.config.js`의 `inlineRem`.)
* **`spacing`에는 rem이 아니라 px로 적는다.** `inlineRem` 환산은 Tailwind 기본 스케일에만 적용되고 `spacing`에 직접 적은 값은 그대로 쓰이므로, px으로 적어야 시안 값이 보존된다. 키 번호는 `키 × 4 = px`에 맞춘다 — `17: "68px"`처럼 기본 스케일의 규칙과 어긋나지 않게 한다.
...
* `tokens.js`의 `boxShadow`에 등록된 토큰만 쓴다(`shadow-card`). Tailwind 기본 `shadow-sm`/`shadow-md`나 arbitrary value는 쓰지 않는다
...
아이콘 팩(라이브러리)을 설치하지 않는다 — 쓰는 아이콘 수가 적고 출처가 여러 곳이라, SVG를 직접 모아서 자체 세트로 관리한다. 원본은 `src/shared/assets/icons/`.
...
* 크기는 `size` prop(기본 24). 원본 viewBox가 섞여 있어 같은 `size`라도 획 굵기가 미세하게 달라 보일 수 있다 — 거슬리면 코드에서 보정하지 말고 Figma에서 그리드를 맞춰 다시 export한다.
* **로고는 아이콘이 아니다.** 색이 고정된 브랜드 자산은 `src/shared/assets/logo/`에 두고 `shared/components/base/Logo.tsx`로 쓴다 — `Icon`은 정사각형(`width=height=size`)을 강제하는데 로고는 가로로 길고, 시안 확정 2색이라 `currentColor` 치환과도 맞지 않는다. `Logo`에는 `color` prop을 두지 않고 자산은 `variant`로 받는다 — 치수·색·variant 목록은 `Logo.tsx`와 `assets/logo/`가 단일 소스다.
...
* 내비게이션 구조: 루트 `Stack.Navigator` 안에 `Tab.Navigator`(메인 탭, BottomNav 포함)를 하나의 스크린으로 넣고, 그 외 화면은 루트 Stack에 push한다.
  ...
  * 어떤 화면이 탭이고 어떤 화면이 push인지는 `BottomTabNavigator.tsx`·`RootNavigator.tsx`가 단일 소스다. 목록을 여기에 복사하지 않는다 — 화면이 늘 때마다 낡는다.
* BottomNav는 `shared/components/base/BottomNav.tsx`에 구현하고 `Tab.Navigator`의 `tabBar` prop으로 넘긴다 — 가운데 말씀 버튼이 탭바 위로 튀어나오고 배경·그림자도 시안 값이라 기본 탭바 옵션으로는 맞출 수 없다.
  ...
  * 탭바 배경·튀어나온 버튼의 터치 영역·그림자는 시안 그대로 맞추느라 계산이 얽혀 있다. 값과 근거는 `BottomNav.tsx` 주석에 있으니 그쪽을 고칠 때만 보면 된다.
