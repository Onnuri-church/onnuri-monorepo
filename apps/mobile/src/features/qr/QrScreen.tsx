...
export function QrScreen() {
  ...
  // TODO(카메라): expo-camera를 붙이기 전이라 스캔을 흉내낸다 — 조준 프레임을 누르면
  // 인식 실패 상태로 바뀐다. 카메라가 붙으면 이 자리가 스캔 콜백이 된다.
  const handleScanPress = () => setInvalid(true);
  ...
  return (
    <View className="flex-1 bg-text-normal" style={{ paddingTop: insets.top }}>
      ...
      <View className="h-8 flex-row items-center justify-between px-5 pt-7">
